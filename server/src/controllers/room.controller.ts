...
export const createRoom = async (req: AuthRequest, res: Response) => {
  try {
    ...
    // Check if candidate exists (optional - depends on requirement, we might just invite by email)
    let candidate = await User.findOne({ email: validatedData.candidateEmail });
    if (!candidate) {
         // Create a placeholder candidate or send invite to register
        candidate = await User.create({
            email: validatedData.candidateEmail,
            role: 'candidate',
            name: validatedData.candidateEmail.split('@')[0],
            passwordHash: 'placeholder', // Handle this gracefully in auth flow
        });
    }
    ...
    const inviteToken = generateInviteToken(room._id);
