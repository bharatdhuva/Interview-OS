...
export const updateProfile = async (req: AuthRequest, res: Response) => {
    try {
        ...
        const userId = req.user?.id as string;
        const user = await User.findByIdAndUpdate(userId, validatedData, { new: true, runValidators: true }).select('-passwordHash -refreshTokens');
        res.status(200).json({ success: true, data: user });
    } catch (error: any) {
...
export const getInterviewHistory = async (req: AuthRequest, res: Response) => {
    try {
        // Can view own, or admin can view anyone
        ...
        const rooms = await InterviewRoom.find({
            $or: [{ interviewer: searchId }, { candidate: searchId }],
            status: 'completed'
        })
