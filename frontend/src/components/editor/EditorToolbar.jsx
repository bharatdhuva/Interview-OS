...
export default function EditorToolbar({ language, onLanguageChange, onRun, onSave, onReset, isRunning, isSaving, lastSavedAt, }) {
    return (<div className="h-11 border-b border-border flex items-center justify-between px-3 bg-background/40 backdrop-blur-sm shrink-0">
      ...
      <div className="flex items-center gap-1.5">
        {/* Reset */}
        <Button size="sm" variant="ghost" className="h-8 text-[11px] gap-1.5 hover:bg-secondary text-muted-foreground" onClick={onReset} disabled={isRunning} title="Reset to starter code">
          <RotateCcw className="w-3.5 h-3.5"/>
          <span className="hidden sm:inline">Reset</span>
        </Button>

        {/* Save */}
        <Button size="sm" variant="ghost" className="h-8 text-[11px] gap-1.5 hover:bg-secondary" onClick={onSave} disabled={isSaving}>
        ...
        {/* Run Code */}
        <Button size="sm" className="h-8 text-[11px] gap-1.5 bg-primary hover:bg-primary/90 shadow-md shadow-primary/20 font-semibold" onClick={onRun} disabled={isRunning}>
