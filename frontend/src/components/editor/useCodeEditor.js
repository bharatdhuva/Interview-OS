...
import { STARTER_CODE, DEFAULT_LANGUAGE, AUTOSAVE_DELAY_MS, MAX_EXECUTIONS_PER_SESSION, } from './constants';
function getStorageKey(roomId) {
    return `interviewos:room:${roomId}:editor`;
}
function loadPersistedState(roomId) {
    try {
...
export function useCodeEditor({ roomId, sessionId, onSaveSuccess, }) {
    // ── Language ─────────────────────────────────────────────
    ...
    const saveCode = useCallback((triggeredBy) => {
        try {
            setIsSaving(true);
            localStorage.setItem(getStorageKey(roomId), JSON.stringify({
                roomId,
                language,
                codeByLanguage,
    ...
    const runCode = useCallback(async () => {
        ...
        setOutput(null);
        try {
            const { data } = await api.post(`/rooms/${roomId}/code/execute`, {
                language,
                code,
                roomId,
...
async function localFallbackExec(language, code, setOutput) {
