...
const AnimatedBackground = () => {
    return (<div className="absolute inset-0 pointer-events-none overflow-hidden">
      ...
      <svg className="absolute" style={{ top: "15%", right: "10%", width: 120, height: 120 }}>
        <motion.circle cx="60" cy="60" r="50" fill="none" stroke="rgba(99,102,241,0.1)" strokeWidth="1" strokeDasharray="8 12" initial={{ rotate: 0 }} animate={{ rotate: 360 }} transition={{ duration: 20, repeat: Infinity, ease: "linear" }} style={{ transformOrigin: "60px 60px" }}/>
        <motion.circle cx="60" cy="60" r="35" fill="none" stroke="rgba(139,92,246,0.08)" strokeWidth="1" strokeDasharray="5 10" initial={{ rotate: 0 }} animate={{ rotate: -360 }} transition={{ duration: 15, repeat: Infinity, ease: "linear" }} style={{ transformOrigin: "60px 60px" }}/>
      </svg>
      ...
      <svg className="absolute" style={{
        ...
        <motion.path d="M40 10 L70 10 L85 36 L70 62 L40 62 L25 36 Z" fill="none" stroke="rgba(99,102,241,0.12)" strokeWidth="1" initial={{ pathLength: 0 }} animate={{ pathLength: 1 }} transition={{ duration: 2, delay: 1.5 }}/>
        <motion.path d="M70 62 L100 62 L115 88 L100 114 L70 114 L55 88 Z" fill="none" stroke="rgba(139,92,246,0.1)" strokeWidth="1" initial={{ pathLength: 0 }} animate={{ pathLength: 1 }} transition={{ duration: 2, delay: 2 }}/>
        <motion.path d="M10 62 L40 62 L55 88 L40 114 L10 114 L-5 88 Z" fill="none" stroke="rgba(6,182,212,0.08)" strokeWidth="1" initial={{ pathLength: 0 }} animate={{ pathLength: 1 }} transition={{ duration: 2, delay: 2.5 }}/>
      </svg>

      {/* Floating data stream */}
      <svg className="absolute" style={{ top: "55%", right: "5%", width: 80, height: 200 }}>
        {[0, 1, 2, 3, 4].map((i) => (<motion.rect key={i} x="30" y={i * 40} width={20 - i * 2} height="2" rx="1" fill={`rgba(99,102,241,${0.2 - i * 0.03})`} initial={{ opacity: 0, x: -10 }} animate={{ opacity: [0, 0.6, 0], x: [0, 10, 20] }} transition={{
                duration: 2,
                delay: 1.5 + i * 0.3,
                repeat: Infinity,
                repeatDelay: 2,
